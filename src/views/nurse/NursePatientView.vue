        ...
        <div class="dashboard-admin">
          ...
          <header class="dashboard-header">
            <div class="header-content">
              ...
              <p class="dashboard-subtitle">Gestion des patients assignés à votre service</p>
            </div>
            ...
            <div class="top-stats">
              <div class="stat-box patients">
                ...
                <div class="stat-details">
                  <h3 class="stat-value">{{ patients.length }}</h3>
              ...
              <div class="stat-box">
                ...
                <div class="stat-details">
                  <h3 class="stat-value">{{ getCriticalPatients() }}</h3>
              ...
              <div class="stat-box">
                ...
                <div class="stat-details">
                  <h3 class="stat-value">{{ getPendingCares() }}</h3>
                  <p class="stat-label">Soins en Attente</p>
                  <div class="stat-trend">
          ...
          <section class="tables-section">
            <div class="table-section">
              <div class="table-header">
                ...
                <span class="badge-count">{{ patients.length }}</span>
              </div>
              ...
              <div v-else-if="patients.length === 0" class="empty-state">
                ...
                <h5>Aucun patient assigné</h5>
                <p>Aucun patient n'est actuellement assigné à votre service</p>
              </div>
                  ...
                  <table class="patients-table">
                    ...
                    <tbody>
                      <tr v-for="patient in patients" :key="patient.id" class="patient-row">
                        ...
                        <td>
                          <div class="care-info">
                            <div class="care-time">{{ patient.lastCare }}</div>
                            <div class="care-status">Effectué</div>
                          </div>
                        </td>
                        <td>
                          <span :class="['status-badge', getPatientStatusClass(patient)]">
                            {{ getPatientStatus(patient) }}
                          </span>
                        </td>
                        <td>
                          <div class="action-buttons">
                            <button class="btn-action primary" @click="viewPatientDetails(patient)">
                              <Eye :size="16" />
                            </button>
                            <button class="btn-action secondary" @click="editPatient(patient)">
                              <Edit :size="16" />
                            </button>
                          </div>
                        </td>
...
import { 
  Users, Activity, Clock, RefreshCw, Bell, CheckCircle, XCircle,
  BarChart3, User, FileText, LogOut, MapPin, Eye, Edit
...
// Patients data
const patients = ref([
  { id: 1, name: 'Fatou T.', gender: 'F', age: 28, room: 'A101', lastCare: 'Pansement 08:30', status: 'stable' },
  { id: 2, name: 'Moussa K.', gender: 'M', age: 34, room: 'B204', lastCare: 'Injection 10:00', status: 'critical' },
  { id: 3, name: 'Bintou S.', gender: 'F', age: 22, room: 'C102', lastCare: 'Température 11:00', status: 'stable' },
  { id: 4, name: 'Ibrahim D.', gender: 'M', age: 45, room: 'A203', lastCare: 'Médicament 09:30', status: 'stable' },
  { id: 5, name: 'Mariam B.', gender: 'F', age: 31, room: 'D105', lastCare: 'Pansement 07:45', status: 'observation' },
])
...
function getCriticalPatients() {
  return patients.value.filter(p => p.status === 'critical').length
}

function getPendingCares() {
  return patients.value.filter(p => p.status === 'observation').length
}
...
function viewPatientDetails(patient) {
  ...
  router.push({ name: 'NursePatientDetails', params: { idNurse: nurseId.value, idPatient: patient.id } })
}
