    ...
    <div class="admin-layout">
              ...
              <div class="title-section">
                ...
                <div class="title-content">
                  <h1 class="dashboard-title">
                    ...
                    {{ isEditing ? 'Modifier' : 'Ajouter' }} un Infirmier
                  </h1>
                  <p class="dashboard-subtitle">
                    {{ isEditing ? 'Mettre à jour les informations de l\'infirmier' : 'Créez un nouveau compte pour le personnel soignant' }}
                  </p>
                </div>
              </div>
      ...
      <!-- Loading State -->
          <div v-if="nurseStore.loading" class="loading-state">
            ...
            <p>{{ isEditing ? 'Modification en cours...' : 'Création en cours...' }}</p>
          </div>
          ...
          <div v-else-if="nurseStore.error" class="error-state">
            ...
            <p>Erreur: <strong>{{ nurseStore.error }}</strong></p>
          </div>
          
          <!-- Success State -->
          <div v-else-if="nurseStore.success" class="success-state">
            <div class="success-icon">
              <CheckCircle :size="48" />
            </div>
            <p>{{ nurseStore.success }}</p>
          </div>

          <!-- Form -->
          <div class="form-section" v-if="!nurseStore.loading && nurseStore.currentNurse">
            <form @submit.prevent="submitForm" class="nurse-form">
              <div class="form-group">
                ...
                <div class="form-row">
                  ...
                  <div class="form-field">
                    <label v-if="isEditing">Nouveau mot de passe</label>
                    <label v-else>Mot de passe</label>
                    <input v-model="form.password" type="password" :placeholder="isEditing ? 'Laisser vide si inchangé' : ''" class="form-input" />
                  </div>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="form-row">
                  <div class="form-field">
                    ...
                    <input v-model="form.speciality" type="text" class="form-input" placeholder="Ex: Soins intensifs" />
                  </div>
                  <div class="form-field">
                ...
                <div class="form-row">
                  ...
                  <div class="form-field">
                    ...
                    <input v-model="form.numero_professionel" type="text" required class="form-input" placeholder="Ex: INF2024001" />
                  </div>
                </div>
                <div class="form-row">
                  <div class="form-field">
                  ...
                  <div class="form-field">
                    ...
                    <input v-model="form.experience" type="number" min="0" max="50" class="form-input" placeholder="Ex: 5" />
                  </div>
                </div>
                <div class="form-field full-width">
              ...
              <div class="form-group">
                ...
                <div class="photo-upload-section">
                  <div class="photo-preview-container" v-if="imagePreview || (isEditing && nurseStore.currentNurse?.user?.profile_photo)">
                    <img :src="imagePreview || `/storage/${nurseStore.currentNurse?.user?.profile_photo}`" alt="Photo de profil" class="photo-preview" />
                    <button type="button" @click="removePhoto" class="remove-photo-btn">
                      <Trash2 :size="16" />
                    </button>
                  </div>
                  <div class="photo-upload-area" :class="{ 'has-photo': imagePreview || (isEditing && nurseStore.currentNurse?.user?.profile_photo) }">
              ...
              <div class="form-actions">
                ...
                <button type="submit" class="action-btn primary" :disabled="nurseStore.loading">
                  <Save :size="16" />
...
const removePhoto = () => {
  ...
  // Si c'est une édition, on marque pour suppression
  if (isEditing.value) {
...
const submitForm = async () => {
  ...
  if (isEditing.value) {
    ...
    await nurseStore.addNurse(formData);
  }
...
const fillFormWithNurseData = (nurse) => {
  if (!nurse || !nurse.user) return;
  
  Object.assign(form, {
    first_name: nurse.user.first_name || '',
    last_name: nurse.user.last_name || '',
    birth_date: nurse.user.birth_date || '',
    ...
    status: nurse.user.status || 'active',
    speciality: nurse.speciality || '',
    numero_professionel: nurse.numero_professionel || '',
    numero_ordre: nurse.numero_ordre || '',
    biography: nurse.biography || '',
...
onMounted(async () => {
  await nurseStore.fetchOneNurse(route.params.id);
  await departmentStore.fetchDepartments();
});

watch(() => nurseStore.currentNurse, fillFormWithNurseData, { deep: true });
</script>
<style scoped>
/* Layout Container */
