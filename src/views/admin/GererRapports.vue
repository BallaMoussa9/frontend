              ...
              <div class="title-section">
                <div class="title-content">
                  ...
                  <p class="dashboard-subtitle">
                    <span v-if="store.pagination?.total">
                      {{ store.pagination.total }} rapports enregistrés
                    </span>
                    <span v-else>
                      Gérez tous les rapports médicaux
                    </span>
                  </p>
                </div>
              </div>
              <div class="header-actions">
                <button class="action-btn primary" @click="addRapport">
                  <Plus :size="16" />
                  Ajouter un rapport
                </button>
              </div>
          ...
          <div class="table-section" v-else>
            <div class="table-container">
              <table class="rapports-table">
                ...
                <tbody>
                  <tr v-for="(rapport, index) in store.medicalReports" :key="rapport.id" class="table-row">
                    <td class="col-id">{{ index + 1 }}</td>
                    <td>
                      <div class="patient-cell">
                        <div class="patient-avatar">
                          {{ getPatientInitials(rapport.patient) }}
                        </div>
                        <div class="patient-info">
                          <div class="patient-name">
                            {{ rapport.patient?.user?.first_name }} {{ rapport.patient?.user?.last_name }}
                          </div>
                          <div class="patient-id">ID: #{{ rapport.patient_id }}</div>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div class="doctor-cell">
                        <div class="doctor-info">
                          <span class="doctor-tag">Dr.</span>
                          {{ rapport.doctor?.user?.last_name || 'Non assigné' }}
                        </div>
                      </div>
                    </td>
                    <td class="date-cell">
                    ...
                    <td>
                      <span class="type-badge">{{ rapport.report_type }}</span>
                    </td>
                    <td class="text-right">
                      <div class="action-buttons">
                        <RouterLink :to="{ name: 'SeenRapport', params: { id: rapport.id } }" 
                                     class="action-btn-icon view" 
                                     title="Voir le rapport">
                          <Eye :size="16" />
                        </RouterLink>
                        <RouterLink :to="{ name: 'EditRapport', params: { id: rapport.id } }" 
                                     class="action-btn-icon edit" 
                                     title="Modifier">
                          <Edit :size="16" />
                        </RouterLink>
                        <button @click="confirmDelete(rapport.id)" 
                  ...
                  <tr v-if="store.medicalReports.length === 0" class="empty-row">
                    <td colspan="6">
                      <div class="empty-state">
                        <div class="empty-icon">
                          <FileText :size="48" />
                        </div>
                        <p>Aucun rapport médical trouvé</p>
                      </div>
                    </td>
                  </tr>
                </tbody>
